...
def W2CGetter(W2C):
    def _f(str):
        if str in W2C:
            return W2C[str]
        elif f"DBPEDIA_ID/{str}" in W2C:
            return W2C[f"DBPEDIA_ID/{str}"]
        else:
            return np.zeros((W2C.vector_size,))
    return _f


def validateOn(fitMethod: str, decomp: str, model_name: str, root = 'results'):
    # for n-gram
    Model: Dict[str, Tuple[Dict[int, float]]] = model(model_name)
    decomper = eval(decomp)
    saver = []
    path = f"{root}/{model_name}/{decomp}/{fitMethod}"
    try:
        os.makedirs(path)
    except:
        pass
...
def validateW2COn(fitMethod: str, model, root = 'w2cresults'):
    # for W2C
    Model = W2CGetter(model)
    saver = []
    path = f"{root}/{fitMethod}"
    try:
        os.makedirs(path)
    except:
        pass
    for entity, relatedEntities, referedOntologies in entities:
        stats_to_here = f"{path}/{entity}"
        pos_entities = list(relatedEntities)
        neg_entities = []
        total_pos = len(pos_entities)
        total_neg = 0
        targets = np.hstack((np.ones((total_pos,)), np.zeros((total_pos,))))
        for neg_entity, neg_ref_ontos in ontology_index.items():
            if not neg_ref_ontos.isdisjoint(referedOntologies) and neg_entity not in relatedEntities:
        ...
        pos_vecs, neg_vecs = [Model(pos_data) for pos_data in pos_entities], \
                             [Model(neg_data) for neg_data in neg_entities]
        ori_names = np.hstack((pos_entities, neg_entities))
        datas = np.vstack((pos_vecs, neg_vecs))
        datas_shuffled = np.take(datas, shuffled, axis=0)
        targets_shuffled = np.take(targets, shuffled, axis=0)
        ori_names_shuffled = np.take(ori_names, shuffled, axis=0)
